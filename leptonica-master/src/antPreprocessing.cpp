...
PIX* imageEnhancement(PIX *pixs) {
	PIX *pix1, *pix2, *pix3;
	int d = pixGetDepth(pixs);
	printf("Depth : %d\n", d);
	if (d == 32) {
		pixs = pixConvertTo32(pixs);
		pix1 = pixConvertRGBToGray(pixs, 0.0, 0.0, 0.0);
	} else if (d == 8) {
		pix1 = pixConvert8To32(pixs);
		pix1 = pixConvertRGBToGray(pix1, 0.0, 0.0, 0.0);
	} else if (d == 1) {
		pix1 = pixConvert1To32(NULL, pixs, 0xffffffff, 0);
		pix1 = pixConvertRGBToGray(pix1, 0.0, 0.0, 0.0);

	}
	pix2 = pixBackgroundNormFlex(pix1, 7, 7, 1, 1, 10);
	pix3 = pixGammaTRCMasked(NULL, pix2, NULL, 1.0, 100, 170);
	return pix3;
}
...
PIX* processImages(PIX *pixs) {
	...
	pix1 = lineRemove(pixk1, horizonal);
	...
	for (i = 0; i < noOfFiles; i++) {
		...
		if (fileFormatIsTiff(fp)) {
			...
			if (npages == 1) {
				...
				pixWrite(tempFilePath, pixConvertTo1(pix, 180), 8);
				pixDestroy(&pix);
			} else {
				for (j = 0; j < npages; j++) { /* read one beyond to catch error */
					...
					tempFilePath = genPathname(outputFolder, buffer);
					pixSetResolution(pix, 200, 200);
					pixWriteTiff(tempFilePath, pixConvertTo1(pix, 180),
							8, "a+");
					pixDestroy(&pix);
				}
			}
